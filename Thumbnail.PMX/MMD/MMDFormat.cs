	...
	namespace PMX
	{
		...
		public class PMXFormat
		{
			...
			public class Vertex
			{
				...
				public Point3D pos; // x, y, z // 座標
				public Point3D normal_vec; // nx, ny, nz // 法線ベクトル
				public Point uv; // u, v // UV座標 // MMDは頂点UV
				public Point4D[] add_uv; // x,y,z,w
			...
			public interface BoneWeight
			{
				...
				Point3D c_value {get;}
				Point3D r0_value {get;}
				Point3D r1_value {get;}
			}
			...
			public class BDEF1 : BoneWeight
			{
				...
				public Point3D c_value {get{return new Point3D(0,0,0);}}
				public Point3D r0_value {get{return new Point3D(0,0,0);}}
				public Point3D r1_value {get{return new Point3D(0,0,0);}}
			}
			...
			public class SDEF : BoneWeight
			{
				...
				public uint bone2_ref {get; set;}
				public float bone1_weight {get; set;}
				public Point3D c_value {get; set;}
				public Point3D r0_value {get; set;}
				public Point3D r1_value {get; set;}
				public uint bone3_ref {get{return 0;}}
				public uint bone4_ref {get{return 0;}}
				public float bone2_weight {get{return 1.0f - bone1_weight;}}
				public float bone3_weight {get{return 0.0f;}}
			...
			public class IK_Link
			{
				...
				public Point3D lower_limit;
				public Point3D upper_limit;
			}
			...
			public class ImpulseMorphOffset : MorphOffset
			{
				...
				public Point3D move_velocity;
				public Point3D rotation_torque;
			}
			...
			public class Rigidbody
			{
				...
				public Point3D shape_size;
				public Point3D collider_position;	 // 位置：位置(x, y, z) 
				public Point3D collider_rotation;	 // 位置：回転(rad(x), rad(y), rad(z)) 
			...
			public class Joint
			{
				...
				public Point3D position; // 諸データ：位置(x, y, z) // 諸データ：位置合せでも設定可 
				public Point3D rotation; // 諸データ：回転(rad(x), rad(y), rad(z)) 
				public Point3D constrain_pos_lower; // 制限：移動1(x, y, z) 
				public Point3D constrain_pos_upper; // 制限：移動2(x, y, z) 
				public Point3D constrain_rot_lower; // 制限：回転1(rad(x), rad(y), rad(z)) 
				public Point3D constrain_rot_upper; // 制限：回転2(rad(x), rad(y), rad(z)) 
				public Point3D spring_position; // ばね：移動(x, y, z) 
				public Point3D spring_rotation; // ばね：回転(rad(x), rad(y), rad(z)) 
			}
		}
	}
	...
	namespace PMD
	{
		...
		public class PMDFormat
		{
			...
			public class Bone
			{
				...
				public Point3D bone_head_pos; // x, y, z // ボーンのヘッドの位置
