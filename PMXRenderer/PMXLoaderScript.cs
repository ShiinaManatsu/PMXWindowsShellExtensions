...
namespace MMDExtensions
{
    public partial class PMXLoaderScript
    {
        ...
        private PMXFormat.Vertex ReadVertex()
        {
            ...
            result.uv = ReadSinglesToVector(binary_reader_);
        ...
        private static Vector2 ReadSinglesToVector(BinaryReader binary_reader_)
        {
            const int count = 2;
            float[] result = new float[count];
            for (int i = 0; i < count; i++)
        ...
        private PMXFormat.Header.StringCode string_code_;

        public static MeshCreationInfo CreateMeshCreationInfoSingle(PMXFormat format)
        {
        ...
        public static MeshCreationInfo.Pack[] CreateMeshCreationInfoPacks(PMXFormat format)
        {
            uint plane_start = 0;
